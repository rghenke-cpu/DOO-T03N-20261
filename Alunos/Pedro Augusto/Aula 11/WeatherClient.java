...
public class WeatherClient {
    ...
    public WeatherData buscar(String cidade) throws WeatherException {
        String json = get(cidade);
        return parse(json, cidade);
    }

    private String get(String cidade) throws WeatherException {
        try {
            String url = URL_BASE
            ...
            WeatherException.verificarStatus(conn.getResponseCode(), cidade);
            ...
            throw new WeatherException("Falha de conexão: " + e.getMessage());
        }
    }

    private WeatherData parse(String json, String cidadeFallback) throws WeatherException {
        try {
            String cidade = texto(json, "resolvedAddress", cidadeFallback);

            int ci = json.indexOf("\"currentConditions\"");
            if (ci == -1) {
                throw new WeatherException("Dados atuais não encontrados na resposta.");
            }
            ...
            String condicao = texto(cur, "conditions", "N/A");
            ...
            throw new WeatherException("Erro ao processar resposta da API.");
        }
    }
